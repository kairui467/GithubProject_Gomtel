...
public class BluetoothServerService extends Service {
	...
	private BroadcastReceiver controlReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			} else if (BluetoothTools.ACTION_REQUEST_DISCOVERABLE_OK.equals(action)) {
				Log.i("gomtel---", "开启蓝牙发现功能（300秒）");
				new BluetoothServerConnThread(serviceHandler).start();// 开启后台连接线程
			}
		}
	};
	...
	public void onCreate() {
		...
		controlFilter.addAction(BluetoothTools.ACTION_REQUEST_DISCOVERABLE_OK);

		// 注册BroadcastReceiver
		registerReceiver(controlReceiver, controlFilter);

		// TODO 放到Activity里面去开启
		// 开启服务器
